...
5. Create the per-registry preview iframe route tree.
   ...
   - `demos/<component-name>/<demo-name>/page.tsx`: import registry components from `@repo/<registry-name>/ui/<component>` so they render with the isolated registry package. Reference it from MDX as `<Preview registry="<registry-name>" demo="<component-name>/<demo-name>" />`. The Code tab will rewrite those imports to `@/components/ui/<component>` for readers. Demos follow the parent docs language: `<Preview>` forwards it as a `?lang=` search param (each registry preview `layout.tsx` mounts `PreviewDictionaryProvider`), so localize static labels via the `"use client"` hook `usePreviewDictionary()` from `@/components/preview/preview-dictionary-provider` (reading the `demos` section of `src/dictionaries/{en,ko}.json`) rather than hardcoding strings. `/api/preview-code` strips the hook and inlines `t.*` to literals, so the Code tab stays copy-paste clean. See the `add-registry-component-docs` skill for the demo i18n pattern.
